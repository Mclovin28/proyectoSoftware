    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            try (Connection conn = ConnectionUtils.getConnection()) {
                String query = "SELECT FechaI, FechaF, Descripcion, ID FROM Fecha"; // Update the query to fetch the user ID
                ...
                while (rs.next()) {
                    ...
                    events.add(new Event(title, start, end, userId)); // Add the user ID to the event object
